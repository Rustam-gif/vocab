...
export function RouteRenderer() {
  ...
  const { View, Animated, Easing, Dimensions, PanResponder, TouchableOpacity, Text } = require('react-native');
  ...
  const overlayPaths = new Set<string>(['/translate', '/story/StoryExercise']);
  ...
  const closeSheet = React.useCallback(() => {
    try {
      ...
      Animated.timing(sheetAnim, { toValue: 0, duration: 240, easing: Easing.in(Easing.cubic), useNativeDriver: true }).start(() => {
        setSheetRoute(null);
        // Pop translate route from stack
        ctx.setStack(s => (s.length > 1 ? s.slice(0, -1) : s));
  ...
  React.useEffect(() => {
    ...
    const open = from?.pathname === '/' && toIsOverlay;
